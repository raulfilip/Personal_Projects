...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def maxval(self, gameState, agentIndex, depth, alpha, beta):
        ...
        for successor in gameState.getLegalActions(agentIndex):
            succAction = (successor, self.alphabeta(gameState.generateSuccessor(agentIndex,successor), (depth + 1) % gameState.getNumAgents(), depth + 1, alpha, beta))
            best_action = max(best_action, succAction, key = lambda x:x[1])

            if beta >= best_action[1]:                 
    ...
    def minval(self, gameState, agentIndex, depth, alpha, beta):
        ...
        for successor in gameState.getLegalActions(agentIndex):
            succAction = (successor,self.alphabeta(gameState.generateSuccessor(agentIndex, successor), (depth + 1) % gameState.getNumAgents(), depth + 1, alpha, beta))
            best_action = min(best_action, succAction, key = lambda x:x[1])

            if alpha <= best_action[1]: 
    ...
    def is_terminal(self, gameState, depth):
        return (depth == self.depth * gameState.getNumAgents()) or gameState.isLose() or gameState.isWin()
